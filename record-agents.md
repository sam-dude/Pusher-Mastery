...
Gymnasium provides two essential wrappers for recording: RecordEpisodeStatistics for numerical data and RecordVideo for visual recordings. The first tracks episode metrics like total rewards, episode length, and time taken. The second generates MP4 videos of agent behavior using environment renderings.
...
When evaluating a trained agent, you typically want to record several episodes to understand average performance and consistency. Here’s how to set this up with RecordEpisodeStatistics and RecordVideo.
...
In the script above, the RecordVideo wrapper saves videos with filenames like “eval-episode-0.mp4” in the specified folder. The episode_trigger=lambda x: True ensures every episode is recorded.

The RecordEpisodeStatistics wrapper tracks performance metrics in internal queues that we access after evaluation to compute averages and other statistics.
...
More Information
Training an agent - Learn how to build the agents you’re recording

Basic usage - Understand Gymnasium fundamentals

More training tutorials - Advanced training techniques

Custom environments - Create your own environments to record
